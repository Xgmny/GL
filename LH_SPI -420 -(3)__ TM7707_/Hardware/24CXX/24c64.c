...
void AT24CXX_Init(void)
{
	...
	if((kl[0]!=0x38)|(kl[1]!=0x36)|(kl[2]!=0x4f)|(kl[3]!=0x4b))
	{
		...
		kl[0]=0x20;kl[1]=0x30;kl[2]=0x31;kl[3]=0x2E;kl[4]=0x30,kl[5]=0x30;kl[6]=0x30;kl[7]=0x30;
		AT24CXX_Write(0x0060,kl,6);//WDZ    改为>35度调节
		AT24CXX_Write(0x0068,kl,8);//模拟量   第8位为模拟量标志位(地址0x0075)
		AT24CXX_Write(0x0070,kl,6);//QJZ   角度正
		AT24CXX_Write(0x0078,kl,6);//QJF   角度负
		kl[0]=0x30;kl[1]=0x33;kl[2]=0x34;kl[3]=0x39;kl[4]=0x30;kl[5]=0x30;kl[6]=0x30;kl[7]=0x30;
	  ...
	  AT24CXX_Write(0x01f0,kl,8);         //计年
		...
		delay_ms(2);
	  AT24CXX_Read(0x0075,kl,1);  //模拟量切换
	  MNL=kl[0];
...
void BL_24c64(void){
    ...
    SZ_LL_F = A_N_24C64(6,kl,0X01D8);   //反量程
		...
		MNL = A_N_24C64(1,kl,0x006F);   //模拟量 模式 切换 
}	
